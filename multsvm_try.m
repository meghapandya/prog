S = size (indian_pines_gt);
indian_pines_gt_i = reshape(indian_pines_gt,[S(1)*S(2),1]);
C =indian_pines_gt_i;
S1 = size(indian_pines_corrected);
indian_pines_corrected_i = reshape(indian_pines_corrected,[S1(1)*S1(2),S1(3)]);
T = indian_pines_corrected_i;
test = indian_pines_gt;
%test = indian_pines_gt;
%test = Test;
%C = GroupTrain;
itrind=size(test,1);
